...
const DASHBOARD_URL = process.env.DE_E2E_DASHBOARD_URL || '/#/panel/index?dvId=985192741891870720'

const CHART_WRAPPER_SELECTOR =
  process.env.DE_E2E_CHART_SELECTOR || '#wrapper-outer-id-985192540141654016'
...
test.describe('Chart Excel export', () => {
  test('dispatches innerExportDetails request without crashing', async ({ page }) => {
    ...
    const [_exportReq] = await Promise.all([
      page.waitForRequest(
