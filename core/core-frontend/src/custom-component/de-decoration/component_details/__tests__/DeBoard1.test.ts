import { shallowMount } from '@vue/test-utils'
import { describe, expect, it, vi } from 'vitest'

vi.mock('@/hooks/web/useI18n', () => ({ useI18n: () => ({ t: (k: string) => k }) }))
vi.mock('@/config/axios/service', () => ({
  service: {} as any,
  PATH_URL: './',
  cancelMap: new Map()
}))
vi.mock('@/custom-component/de-decoration/component_details/config', () => ({
  customMergeColor: (defaults: string[], colors: string[]) =>
    defaults.map((d, i) => (colors && colors[i] != null ? colors[i] : d))
}))

import DeBoard1 from '../DeBoard1.vue'

const createProps = (overrides = {}) => ({
  curStyle: { width: 320, height: 240 },
  scale: 1,
  ...overrides
})

describe('DeBoard1.vue', () => {
  it('renders the component', () => {
    const wrapper = shallowMount(DeBoard1, { props: createProps() })
    expect(wrapper.exists()).toBe(true)
  })

  it('renders the root container with correct class', () => {
    const wrapper = shallowMount(DeBoard1, { props: createProps() })
    expect(wrapper.find('.dv-border-box-1').exists()).toBe(true)
  })

  it('renders SVG elements', () => {
    const wrapper = shallowMount(DeBoard1, { props: createProps() })
    expect(wrapper.find('svg').exists()).toBe(true)
  })

  it('renders the border-box-content slot container', () => {
    const wrapper = shallowMount(DeBoard1, { props: createProps() })
    expect(wrapper.find('.border-box-content').exists()).toBe(true)
  })

  it('applies style based on curStyle and scale', () => {
    const wrapper = shallowMount(DeBoard1, {
      props: createProps({ curStyle: { width: 500, height: 300 }, scale: 2 })
    })
    const style = wrapper.find('.dv-border-box-1').attributes('style')
    expect(style).toContain('500px')
    expect(style).toContain('300px')
    expect(style).toContain('scale(2)')
  })

  it('uses default colors when no color prop provided', () => {
    const wrapper = shallowMount(DeBoard1, { props: createProps() })
    expect(wrapper.exists()).toBe(true)
  })

  it('accepts custom colors', () => {
    const wrapper = shallowMount(DeBoard1, {
      props: createProps({ color: ['#ff0000', '#00ff00'] })
    })
    expect(wrapper.exists()).toBe(true)
  })

  it('accepts backgroundColor prop', () => {
    const wrapper = shallowMount(DeBoard1, {
      props: createProps({ backgroundColor: '#000000' })
    })
    expect(wrapper.exists()).toBe(true)
  })

  it('uses default props correctly', () => {
    const wrapper = shallowMount(DeBoard1, {
      props: { curStyle: { width: 100, height: 100 }, scale: 1 }
    })
    expect(wrapper.exists()).toBe(true)
  })
})
