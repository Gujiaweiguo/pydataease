...
import DeBoard6 from '../DeBoard6.vue'
...
describe('DeBoard6.vue', () => {
  it('renders the component', () => {
    const wrapper = shallowMount(DeBoard6, { props: createProps() })
    expect(wrapper.exists()).toBe(true)
  })

  it('renders the root container with correct class', () => {
    const wrapper = shallowMount(DeBoard6, { props: createProps() })
    expect(wrapper.find('.dv-border-box-6').exists()).toBe(true)
  })
  ...
  it('applies style based on curStyle and scale', () => {
    const wrapper = shallowMount(DeBoard6, {
      props: createProps({ curStyle: { width: 500, height: 350 }, scale: 0.8 })
    })
    const style = wrapper.find('.dv-border-box-6').attributes('style')
    expect(style).toContain('scale(0.8)')
  })

  it('accepts custom colors', () => {
    const wrapper = shallowMount(DeBoard6, {
      props: createProps({ color: ['#555555', '#666666'] })
    })
    expect(wrapper.exists()).toBe(true)
  })
})
