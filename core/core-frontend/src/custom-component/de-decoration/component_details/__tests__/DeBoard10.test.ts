...
import DeBoard10 from '../DeBoard10.vue'
...
describe('DeBoard10.vue', () => {
  it('renders the component', () => {
    const wrapper = shallowMount(DeBoard10, { props: createProps() })
    expect(wrapper.exists()).toBe(true)
  })

  it('renders the root container with correct class', () => {
    const wrapper = shallowMount(DeBoard10, { props: createProps() })
    expect(wrapper.find('.dv-border-box-10').exists()).toBe(true)
  })
  ...
  it('applies style with box-shadow', () => {
    const wrapper = shallowMount(DeBoard10, {
      props: createProps({ curStyle: { width: 600, height: 400 }, scale: 1.5 })
    })
    const style = wrapper.find('.dv-border-box-10').attributes('style')
