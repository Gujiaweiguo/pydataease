...
import DeBoard3 from '../DeBoard3.vue'
...
describe('DeBoard3.vue', () => {
  it('renders the component', () => {
    const wrapper = shallowMount(DeBoard3, { props: createProps() })
    expect(wrapper.exists()).toBe(true)
  })

  it('renders the root container with correct class', () => {
    const wrapper = shallowMount(DeBoard3, { props: createProps() })
    expect(wrapper.find('.dv-border-box-3').exists()).toBe(true)
  })
  ...
  it('applies style based on curStyle and scale', () => {
    const wrapper = shallowMount(DeBoard3, {
      props: createProps({ curStyle: { width: 400, height: 300 }, scale: 0.5 })
    })
    const style = wrapper.find('.dv-border-box-3').attributes('style')
