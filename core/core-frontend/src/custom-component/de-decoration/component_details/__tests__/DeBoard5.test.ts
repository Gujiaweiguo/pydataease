...
import DeBoard5 from '../DeBoard5.vue'
...
describe('DeBoard5.vue', () => {
  it('renders the component', () => {
    const wrapper = shallowMount(DeBoard5, { props: createProps() })
    expect(wrapper.exists()).toBe(true)
  })

  it('renders the root container with correct class', () => {
    const wrapper = shallowMount(DeBoard5, { props: createProps() })
    expect(wrapper.find('.dv-border-box-5').exists()).toBe(true)
  })
  ...
  it('applies style based on curStyle and scale', () => {
    const wrapper = shallowMount(DeBoard5, {
      props: createProps({ curStyle: { width: 700, height: 500 }, scale: 1.5 })
    })
    const style = wrapper.find('.dv-border-box-5').attributes('style')
    expect(style).toContain('700px')
    expect(style).toContain('scale(1.5)')
  })

  it('accepts custom colors', () => {
    const wrapper = shallowMount(DeBoard5, {
      props: createProps({ color: ['#333333', '#444444'] })
    })
    expect(wrapper.exists()).toBe(true)
  })
})
