...
vi.mock('tinymce/tinymce', () => {
  const mock = { init: vi.fn(), editors: {}, activeEditor: { selection: { getNode: vi.fn() } }, get: vi.fn() }
  return { default: mock, __esModule: true }
})
vi.mock('@tinymce/tinymce-vue', () => ({ default: { name: 'Editor', template: '<div class="tinymce-editor-stub" />', props: ['modelValue', 'init', 'disabled', 'id'] } }))
...
vi.mock('@/store/modules/data-visualization/dvMain', () => ({
  dvMainStoreWithOut: () => ({ dvInfo: { type: 'dashboard' }, canvasViewInfo: {}, mobileInPc: false, setViewDataDetails: vi.fn() })
}))
vi.mock('@/store/modules/appearance', () => ({ useAppearanceStoreWithOut: () => ({ fontList: [], setCurrentFont: vi.fn() }) }))
...
vi.mock('@/api/chart', () => ({ getData: () => Promise.resolve({ data: { fields: [], tableRow: [], sourceFields: [] } }) }))
vi.mock('@/hooks/web/useEmitt', () => ({ useEmitt: () => ({ emitter: { emit: vi.fn(), on: vi.fn() } }) }))
vi.mock('pinia', async (importOriginal) => {
  const actual = await importOriginal() as any
  return { ...actual, storeToRefs: () => ({ canvasViewInfo: { value: {} }, mobileInPc: { value: false } }) }
})

import DeRichTextView from '../DeRichTextView.vue'

const stubs = {
  Editor: { template: '<div class="editor-stub" />', props: ['modelValue', 'init', 'disabled', 'id'] },
  'chart-error': { template: '<div class="chart-error" />', props: ['errMsg'] }
}

const createElement = { id: 'rich-text-1', propValue: { textValue: '<p>Hello</p>', verticalAlign: 'top-align' }, style: {}, canvasId: 'canvas-main' }

const mountRichText = (propsOverrides: Record<string, any> = {}) =>
  shallowMount(DeRichTextView, {
    props: { scale: 1, element: createElement, editMode: 'edit', active: false, disabled: false, showPosition: 'preview', themes: 'dark', suffixId: 'common', ...propsOverrides },
    global: { stubs }
  })

describe('DeRichTextView', () => {
  it('renders successfully with default props', () => { expect(mountRichText().exists()).toBe(true) })
  it('renders the rich-main-class container', () => { expect(mountRichText().find('.rich-main-class').exists()).toBe(true) })
  it('does not have edit-model class initially', () => { expect(mountRichText().find('.edit-model').exists()).toBe(false) })
  ...
  it('does not render chart-error when isError is false', () => { expect(mountRichText().find('.chart-error').exists()).toBe(false) })
})
