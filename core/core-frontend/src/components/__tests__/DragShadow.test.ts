...
describe('DragShadow', () => {
  it('computes placement and size styles', () => {
  ...
  it('renders the background helper node', () => {
    const wrapper = mountComponent()

    expect(wrapper.find('.shadow-background').exists()).toBe(true)
  })

  it('reacts to element updates', async () => {
    const wrapper = mountComponent()

    await wrapper.setProps({ element: { x: 1, y: 1, sizeX: 2, sizeY: 2 } })
