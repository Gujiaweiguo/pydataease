...
import DatasetParamsSettingDialog from '../DatasetParamsSettingDialog.vue'
...
const mountComponent = (linkInfo: Record<string, unknown>) =>
  shallowMount(DatasetParamsSettingDialog, {
    props: { linkInfo },
    global: { stubs }
  })
...
describe('DatasetParamsSettingDialog', () => {
  ...
  it('disables open mode and link input when jump is disabled', () => {
  ...
  it('shows tips text when jump is enabled', () => {
    const wrapper = mountComponent({ content: 'a.com', enable: true, openMode: '_self' })

    expect(wrapper.text()).toContain('visualization.link_open_tips')
  })

  it('writes normalized hyperlinks and records snapshot on submit', async () => {
